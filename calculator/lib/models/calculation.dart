...
class Calculation {
  ...
  void splite() {
    ...
    for (int i = 0; i < _input!.length; i++) {
      if (checkSign(_input![i])) {
        if (!temp.isEmpty) _list.add(temp);
        temp = "";
        _list.add(_input![i]);
      } else
        temp += _input![i];
    ...
    for (int i = 0; i < temp.length; i++) {
      if (checkSign(temp[i])) {
        ...
        if (stack.isEmpty)
          ...
          stack.push(
              calculation(double.parse(stack.pop()), double.parse(n2), temp[i])
                  .toString());
      } else
        stack.push(temp[i]);
    }
  ...
  bool checkSign(String input) {
    if (searchValue(input) != -1) return true;
    return false;
  }

  int? searchValue(String temp) {
    for (int i = 0; i < sign.length; i++) {
      if (sign[i].sign!.compareTo(temp) == 0) return sign[i].value;
    }
    return -1;
  }
  ...
  List<String> convertToPostfix() {
    ...
    for (int i = 0; i < _list.length; i++) {
      if (searchValue(_list[i]) != -1) {
        // if()
        while (!stack.isEmpty &&
            searchValue(_list[i])! <= searchValue(stack.top())!) {
          temp.add(stack.pop());
        }
        stack.push(_list[i]);
      } else {
